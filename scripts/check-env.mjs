...
const useNormalizedRepository = truthy.has((process.env.KORAMA_USE_NORMALIZED_REPOSITORY ?? "").toLowerCase());
const errors = [];
const warnings = [];
...
if (useNormalizedRepository && !useSupabase) errors.push("KORAMA_USE_NORMALIZED_REPOSITORY=true requires KORAMA_USE_SUPABASE=true");
if (useNormalizedRepository && !useSupabaseAuth) errors.push("KORAMA_USE_NORMALIZED_REPOSITORY=true requires KORAMA_USE_SUPABASE_AUTH=true");
...
if (production) {
  ...
  if (!useNormalizedRepository) errors.push("Production mode requires KORAMA_USE_NORMALIZED_REPOSITORY=true");
...
console.log(`environment check passed (${production ? "production" : "local"} mode; Supabase ${useSupabase ? "enabled" : "disabled"}; Auth ${useSupabaseAuth ? "enabled" : "demo"}; Repository ${useNormalizedRepository ? "normalized" : "snapshot"}; Paystack ${present("PAYSTACK_SECRET_KEY") ? "enabled" : "deterministic"}; Mapbox ${present("NEXT_PUBLIC_MAPBOX_TOKEN") ? "enabled" : "static fallback"})`);
