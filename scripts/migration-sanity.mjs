...
const migrationFiles = ["0001_korama_foundation.sql", "20260829180421_phase2_complete_foundation.sql"];
const sql = (await Promise.all(migrationFiles.map((file) => readFile(new URL(`../supabase/migrations/${file}`, import.meta.url), "utf8")))).join("\n");
...
const requiredPolicies = ["customers_read_own_orders", "staff_read_scoped_orders", "safety_read_scoped_sorties", "roles_are_not_self_editable", "customers_write_own_carts", "staff_read_scoped_tasks", "safety_read_scoped_weather"];
