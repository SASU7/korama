...
const requiredPolicies = ["customers_read_own_orders", "staff_read_scoped_orders", "safety_read_scoped_sorties", "roles_are_not_self_editable", "customers_write_own_carts", "staff_read_scoped_tasks", "safety_read_scoped_weather", "payment_attempts_server_only", "korama_authenticated_postgres_changes", "demo_state_server_only"];
...
for (const table of ["operating_companies", "markets", "products", "orders", "sorties", "audit_events", "demo_state_snapshots"]) if (!sql.includes(`alter table public.${table} enable row level security`)) throw new Error(`RLS not enabled: ${table}`);
