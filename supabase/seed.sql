...
insert into public.inventory_batches (id, reference, product_id, site_id, operating_company_id, inventory_class, expiry_date, quantity, allocated, quarantined, customs_cleared, origin_supported) values
  ...
  -- Only the shea balm and the blender used to carry stock, because the demo
  -- only ever bought shea balm. A multi-line cart containing anything else
  -- could never be allocated. AF-COCOA-GRANOLA is deliberately left without a
  -- batch: it is the fixture that proves allocation is all-or-nothing.
