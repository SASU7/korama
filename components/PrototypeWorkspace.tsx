...
export type Surface =
  "shop" | "operations" | "compliance" | "delivery" | "markets";
type Quote = {
...
const originLabels: Record<Product["origin"], string> = {
  ...
  ghana_origin_export: "Ghana-origin export",
  marketplace_future: "Marketplace · future",
};
...
const marketRows = [
  {
    ...
    phase: "Phase 1",
    status: "Transacting",
    detail: "Home base · Tema staging · GHS",
  },
  {
    code: "NG",
    name: "Nigeria",
    phase: "Phase 1",
    status: "Transacting",
    detail: "Anchor market · Lekki · NGN",
  },
  {
    code: "CI",
    name: "Côte d’Ivoire",
    phase: "Phase 2",
    status: "Roadmap",
    detail: "Abidjan · CFA franc · French localization",
  },
  {
    code: "SN",
    name: "Senegal",
    phase: "Phase 2",
    status: "Roadmap",
    detail: "Dakar · CFA franc · French localization",
  },
  {
    code: "TG / BJ",
    name: "Togo + Benin",
    phase: "Phase 3",
    status: "Roadmap",
    detail: "Lomé / Cotonou · distribution nodes",
  },
  {
    code: "GN",
    name: "Guinea",
    phase: "Phase 4",
    status: "Roadmap",
    detail: "Conakry · later launch review",
  },
];
...
function OriginBadge({ origin }: { origin: Product["origin"] }) {
  return (
    <Badge
      tone={
        origin === "ghana_origin_export"
          ? "green"
...
function SectionTitle({
  ...
  return (
    <div className="workspace-title">
      ...
      {detail && <p className="workspace-detail">{detail}</p>}
    </div>
  );
}
...
export default function PrototypeWorkspace({
    ...
    fetch("/api/cart/quote", {
      ...
      .then(async (response) => {
        ...
        if (!response.ok || !body.quote)
          throw new Error(
            body.error ?? "Checkout quote could not be calculated",
          );
        return body.quote;
      })
      .then((nextQuote) => {
        if (requestId === quoteRequestRef.current) {
          setServerQuote(nextQuote);
          setState((current) => ({
            ...current,
            cart: [{ productId: selectedProductId, quantity }],
          }));
        }
      })
      .catch((caught) => {
        ...
        if (requestId === quoteRequestRef.current) {
          ...
          setQuoteError(
            caught instanceof Error
              ? caught.message
              : "Checkout quote could not be calculated",
          );
        }
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    const channel = client
      .channel("korama-private-events", { config: { private: true } })
      .on(
        "postgres_changes",
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/delivery/sorties/${state.order?.reference}/command`,
          {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ command: "advance" }),
          },
        );
        ...
        setError(
          caught instanceof Error
            ? caught.message
            : "Telemetry playback could not start",
        );
      }
    }, 900);
  ...
  async function pay() {
    if (!authenticated) {
      router.push("/auth/sign-in?next=/shop");
      return;
    }
    if (!navigator.onLine) {
      setError("You are offline. Reconnect before starting test payment.");
      return;
    }
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Enter a valid Nigerian delivery address",
      );
      return;
    }
    if (!currentQuote) {
      setError(
        quoteError ||
          "Wait for the server checkout quote before starting payment.",
      );
      return;
    }
    setBusy(true);
    ...
    setNotice("");
    try {
      const init = await fetch("/api/payments/paystack/initialize", {
        method: "POST",
        headers: {
      ...
      const initBody = (await readApiBody(init)) as {
        error?: string;
        authorizationUrl?: string;
      };
      if (!init.ok || !initBody.authorizationUrl)
        throw new Error(initBody.error ?? "Payment could not initialize");
      window.location.assign(initBody.authorizationUrl);
    } catch (caught) {
  ...
  async function retryState() {
    if (!navigator.onLine) {
      setError("You are offline. Reconnect before refreshing the application.");
      return;
    }
    setError("");
    try {
  ...
  const changeMarket = (next: "NG" | "GH") => {
    setMarket(next);
    const first = state.products.find((product) => product.market === next);
    ...
    () =>
      state.products.filter(
        (product) =>
          ...
          `${product.name} ${product.producer} ${product.origin}`
            .toLowerCase()
            .includes(search.toLowerCase()),
      ),
    [state.products, search, category, market],
  );
  const isDeepOrder = Boolean(state.order);
  const currentQuote = state.order
    ? {
        subtotalMinor: state.order.subtotalMinor,
        taxMinor: state.order.taxMinor,
        deliveryMinor: state.order.deliveryMinor,
        totalMinor: state.order.totalMinor,
...
function ShopSurface({
  ...
  const addressErrors = {
    recipientName:
      address.recipientName.trim().length < 2 ? "Enter the recipient name" : "",
    addressLine:
      address.addressLine.trim().length < 5 ? "Enter a delivery address" : "",
    city: address.city.trim().length < 2 ? "Enter the delivery city" : "",
  };
  return (
    <>
      <div className="market-picker standalone">
        <label htmlFor="market-picker">Market</label>
        <select
          id="market-picker"
          value={market}
          onChange={(event) => setMarket(event.target.value as "NG" | "GH")}
        >
      ...
      <SectionTitle
        eyebrow={`${market === "NG" ? "Nigeria" : "Ghana"} shop`}
        title="Goods with a traceable journey."
        detail={`Local ${market === "NG" ? "NGN" : "GHS"} pricing · Producer and provenance details included`}
      />
        ...
        <div className="catalogue-column">
          <div className="shop-toolbar">
            <label className="search-box">
              ...
              <input
                ...
                placeholder="Search by product, maker, or provenance"
              />
            </label>
            <div
          ...
          <div className="product-grid">
            {products.map((product) => (
              ...
              >
                <div
                  className={`product-art art-${product.category.toLowerCase().replace(/[^a-z]/g, "-")}`}
                  aria-hidden="true"
                />
        ...
        <div className="detail-column">
          <div className="detail-panel">
            ...
            <div className="detail-content">
              <OriginBadge origin={selectedProduct.origin} />
              <h3>{selectedProduct.name}</h3>
              <p>{selectedProduct.description}</p>
              <div className="detail-facts">
                <div>
                  <small>Producer</small>
                  <strong>{selectedProduct.producer}</strong>
                </div>
                <div>
                  <small>Fulfilment</small>
                  <strong>{selectedProduct.stockLabel}</strong>
                </div>
                <div>
                  <small>Batch / expiry</small>
                  <strong>
                    {selectedProduct.batch} · {selectedProduct.expiry}
                  </strong>
                </div>
              </div>
              {selectedProduct.transformation && (
                <div className="evidence-strip">
                  <ShieldCheck size={15} aria-hidden="true" />
                  <span>
                    <strong>Transformation recorded</strong>
                    <small>{selectedProduct.transformation}</small>
                  </span>
                </div>
              )}
                ...
                selectedProduct.market === "NG" && (
                  <fieldset className="checkout-address">
                    ...
                    <input
                      ...
                      spellCheck={false}
                      aria-invalid={
                        addressAttempted && Boolean(addressErrors.recipientName)
                      }
                      aria-describedby={
                        addressAttempted && addressErrors.recipientName
                    ...
                    />
                    {addressAttempted && addressErrors.recipientName && (
                      <p id="recipient-name-error" className="field-error">
                        {addressErrors.recipientName}
                      </p>
                    )}
                    <label htmlFor="address-line">Delivery address</label>
                    <input
                      id="address-line"
                      name="address-line"
                      type="text"
                      ...
                      spellCheck={false}
                      aria-invalid={
                        addressAttempted && Boolean(addressErrors.addressLine)
                      }
                      aria-describedby={
                        addressAttempted && addressErrors.addressLine
                          ? "address-line-error"
                          : undefined
                      }
                    ...
                    />
                    {addressAttempted && addressErrors.addressLine && (
                      <p id="address-line-error" className="field-error">
                    ...
                    <input
                      ...
                      spellCheck={false}
                      aria-invalid={
                        addressAttempted && Boolean(addressErrors.city)
                      }
                      aria-describedby={
                    ...
                    />
                    {addressAttempted && addressErrors.city && (
                      <p id="delivery-city-error" className="field-error">
                        {addressErrors.city}
                      </p>
                    )}
                    <p className="field-help">
                      Nigeria · NGN · illustrative delivery details
                    </p>
                  </fieldset>
                )}
              {selectedProduct.purchasable &&
                  ...
                  <div className="quote-summary" aria-live="polite">
                    {quoteLoading ? (
                      ...
                      <>
                        ...
                        <small>
                          Includes illustrative tax/duty treatment and delivery.
                        </small>
                      </>
                    ) : (
                      <span>
                        Server quote unavailable. Retry the connection.
                      </span>
                    )}
                  </div>
                )}
              ...
              {selectedProduct.purchasable &&
                selectedProduct.market === "NG" && (
                  <button
                    ...
                    onClick={pay}
                    disabled={
                      busy || Boolean(state.order) || quoteLoading || !quote
                    }
                    aria-busy={busy}
                  >
                    {busy
              ...
              {selectedProduct.purchasable &&
                selectedProduct.market !== "NG" && (
                  <div className="disabled-note">
                    Checkout is currently available in Nigeria.
                  </div>
                )}
              {selectedProduct.origin === "marketplace_future" && (
                <div className="disabled-note">
                  Marketplace settlement is not yet available.
                </div>
              )}
            </div>
          </div>
        </div>
        {isDeepOrder && state.order && quote && (
          <OrderCard state={state} quote={quote} />
        )}
      </div>
    </>
  );
...
function OrderCard({ state, quote }: { state: DemoState; quote: Quote }) {
  return (
    <div className="order-card">
      <div className="order-card-header">
        <div>
          <p className="eyebrow">Order · {state.order?.reference}</p>
          <h3>Ghana → Nigeria</h3>
        </div>
        <Badge tone="green">{state.order?.status.replace("_", " ")}</Badge>
      </div>
      <div className="order-price-lines">
        <span>
          Product <strong>{formatMoney(quote.subtotalMinor, "NGN")}</strong>
        </span>
        <span>
          Illustrative tax/duty treatment{" "}
          <strong>{formatMoney(quote.taxMinor, "NGN")}</strong>
        </span>
        <span>
          Delivery <strong>{formatMoney(quote.deliveryMinor, "NGN")}</strong>
        </span>
        <span className="total-line">
          Total <strong>{formatMoney(quote.totalMinor, "NGN")}</strong>
        </span>
      </div>
      {state.order?.address && (
        <p className="order-address">
          <strong>Deliver to</strong> {state.order.address.recipientName} ·{" "}
          {state.order.address.addressLine}, {state.order.address.city}
        </p>
      )}
      <div className="timeline">
        {state.orderEvents.map((event) => (
          <div
            className={
              event.complete ? "timeline-item complete" : "timeline-item"
            }
            key={event.status}
          >
            <span className="timeline-dot">
              {event.complete && <Check size={11} aria-hidden="true" />}
            </span>
            <span>
...
function OperationsSurface({
  ...
  return (
    <>
      <SectionTitle
        eyebrow="Operations · Lekki"
        title="Make the handoffs legible."
        detail="Warehouse tasks · FEFO allocation · Shared order state"
      />
      {!state.order ? (
        <div className="panel-empty large">
          <Warehouse size={30} aria-hidden="true" />
          <strong>Waiting for a paid order</strong>
          <p>
        ...
        <div className="operations-grid">
          <div className="panel task-panel">
            <div className="panel-header">
              <div>
                <p className="eyebrow">Warehouse task list</p>
                <h3>{state.order.reference}</h3>
              </div>
              <Badge tone={state.order.status === "paid" ? "amber" : "green"}>
                {state.order.status.replace("_", " ")}
              </Badge>
            </div>
            {state.tasks.map((task, index) => (
              ...
              >
                ...
                {index === 1 && state.order?.status === "paid" && (
                  <button
                    type="button"
                    className="small-action"
                    disabled={busy}
                    onClick={() =>
                      mutate(
                        `/api/fulfilment/orders/${ref}/allocate`,
                        { method: "POST" },
                        "FEFO allocated NK-SB-2407: earliest valid stock, no negative balance.",
                      )
                    }
                  >
                    Allocate
                ...
                {index === 2 && state.order?.status === "allocated" && (
                  <button
                    type="button"
                    className="small-action"
                    disabled={busy}
                    onClick={() =>
                      mutate(
                        `/api/orders/${ref}/advance`,
                        {
                          method: "POST",
                          headers: { "content-type": "application/json" },
                          body: JSON.stringify({ status: "picked" }),
                        },
                        "Pick confirmed by scan.",
                      )
                    }
                  >
                    Confirm
                ...
                {index === 3 && state.order?.status === "picked" && (
                  <button
                    type="button"
                    className="small-action"
                    disabled={busy}
                    onClick={() =>
                      mutate(
                        ...
                        "Pack complete: weight captured for routing.",
                      )
                    }
                  >
                    Confirm
                ...
                {index === 4 && state.order?.status === "packed" && (
                  <button
                    type="button"
                    className="small-action"
                    disabled={busy}
                    onClick={() =>
                      mutate(
                        ...
                        "Parcel dispatched to simulated last-mile delivery.",
                      )
                    }
                  >
                    Dispatch
              ...
              {state.batches.map((batch) => (
                <div key={batch.id} className="batch-row">
                  <span>
                    ...
                    <small>
                      {batch.quantity - batch.allocated} units · {batch.site}
                    </small>
                  </span>
                  <span>{batch.expiry}</span>
                  <span>
                    {batch.expiry < "2026-08-29" ? (
                      <Badge tone="amber">Expired · reject</Badge>
                    ) : batch.quarantined ? (
                      <Badge tone="amber">Quarantined</Badge>
                    ) : batch.allocated ? (
          ...
          <div className="panel transfer-panel">
            <div className="panel-header">
              <div>
                ...
                <h3>Auditable Ghana → Lekki movement</h3>
              </div>
              <Truck size={19} aria-hidden="true" />
            </div>
            <div className="transfer-line">
              {state.transfer.map((step) => (
                <div className="transfer-step" key={step.label}>
...
function ComplianceSurface({ state }: { state: DemoState }) {
  return (
    <>
      <SectionTitle
        eyebrow="Compliance · provisional"
        title="Evidence before origin claims."
        detail="Illustrative assessment · Watermarked preview · No legal certificate"
      />
      <div className="compliance-grid">
        <div className="panel origin-panel">
          <div className="assessment-status">
            ...
            <span>
              ...
              <strong>Provisionally eligible</strong>
            </span>
            <Badge tone="amber">Preview</Badge>
          </div>
          <div className="origin-rule">
            <strong>Why this qualifies</strong>
            <p>{state.compliance.transformation}</p>
          </div>
          <h3>Evidence chain</h3>
          {state.compliance.evidence.map((item, index) => (
          ...
          <div className="rejection-note">
            ...
            <span>
              Repackaging or relabelling alone would produce a rejected
              assessment.
            </span>
          </div>
        </div>
        <div className="certificate-card">
          <div className="watermark">
            {state.compliance.certificateWatermark}
          </div>
          <div className="certificate-head">
            <FileCheck2 size={22} aria-hidden="true" />
            <span>Origin assessment preview</span>
          </div>
          <p className="certificate-kicker">ECOWAS / AfCFTA · Illustrative</p>
          <h3>Ghana-origin export record</h3>
          <div className="certificate-fields">
            <span>
              Product <strong>Nokware shea repair balm</strong>
            </span>
            <span>
              Batch <strong>NK-SB-2407</strong>
            </span>
            <span>
              Movement <strong>Tema → Lekki</strong>
            </span>
            <span>
              Duty quote <strong>{state.compliance.dutyQuote}</strong>
            </span>
          </div>
...
function DeliverySurface({
  ...
  const deliveryReady = state.order?.status === "dispatched";
  return (
    <>
      <SectionTitle
        eyebrow="Delivery · simulated digital twin"
        title="Safety gates the last mile."
        detail="Lekki → fictional micro-hub · Static route · No real aircraft"
      />
      {!state.order ? (
        <div className="panel-empty large">
          <Plane size={30} aria-hidden="true" />
          <strong>Waiting for a packed order</strong>
          <p>
            The delivery twin activates after the operator packs the shea
            parcel.
          </p>
        </div>
      ) : (
        <div className="delivery-grid">
          <div className="map-panel">
            <div className="map-top">
              <span>
                <Map size={16} aria-hidden="true" /> Static route preview
              </span>
              <Badge tone="amber">Simulated</Badge>
            </div>
            <MapboxRoutePreview />
            <TelemetryReadout sortie={sortie} />
          </div>
          <div className="panel gate-panel">
            <div className="panel-header">
              <div>
                <p className="eyebrow">Preflight decision</p>
                <h3>{sortie.status.replace("_", " ")}</h3>
              </div>
              <Fuel size={19} aria-hidden="true" />
            </div>
            <div className="gate-list">
              {sortie.gates.map((gate) => (
                <div
                  className={
                    gate.passed ? "gate-row passed" : "gate-row failed"
                  }
                  key={gate.key}
                >
                  <span>
            ...
            {!deliveryReady && (
              <p className="disabled-note">
                Delivery controls unlock after the warehouse dispatches the
                packed order.
              </p>
            )}
            <div className="delivery-actions">
              {sortie.status === "draft" && (
                <button
                  ...
                  onClick={() =>
                    mutate(
                      `/api/delivery/sorties/${ref}/command`,
                      {
                        method: "POST",
                        headers: { "content-type": "application/json" },
                        body: JSON.stringify({ command: "preflight" }),
                      },
                      "All safety gates passed. Mission cleared.",
                    )
                  }
                >
                  Run preflight <ArrowRight size={16} aria-hidden="true" />
                </button>
              )}
                  ...
                  onClick={() =>
                    mutate(
                      `/api/delivery/sorties/${ref}/command`,
                      {
                        method: "POST",
                        headers: { "content-type": "application/json" },
                        body: JSON.stringify({ command: "launch" }),
                      },
                      "Mission launched. Telemetry playback is running.",
                    )
                  }
                >
                  Launch simulated sortie <Plane size={16} aria-hidden="true" />
                </button>
              )}
              <button
                type="button"
                ...
                onClick={() =>
                  mutate(
                    `/api/delivery/sorties/${ref}/command`,
                    {
                      ...
                      body: JSON.stringify({
                        command:
                          sortie.weather === "unsafe"
                            ? "reset_weather"
                            : "inject_weather",
                      }),
                    },
                    sortie.weather === "unsafe"
                      ? "Weather cleared; sortie reset for another run."
                      : "Unsafe weather locked out flight and created a courier fallback.",
                  )
                }
              >
                {sortie.weather === "unsafe" ? (
                ...
                <button
                  ...
                  onClick={() =>
                    mutate(
                      ...
                      "Manual override created a ground-courier handover.",
                    )
                  }
                >
            ...
            {sortie.fallbackReason && (
              <div className="fallback-note">
                <Truck size={15} aria-hidden="true" />
                <span>
                  <strong>Ground-courier fallback</strong>
                  <small>{sortie.fallbackReason}</small>
                </span>
              </div>
            )}
          </div>
...
function TelemetryReadout({ sortie }: { sortie: DemoState["sortie"] }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () =>
        setPointIndex((current) => Math.min(current + 1, points.length - 1)),
      900,
    );
  ...
  return (
    ...
    >
      <span>
        <strong>{point?.altitude ?? 0}m</strong>
        <small>Altitude</small>
      </span>
      <span>
        <strong>{point?.speed ?? 0}km/h</strong>
        <small>Speed</small>
      </span>
      <span>
        <strong>{point?.battery ?? 94}%</strong>
        <small>Battery</small>
      </span>
      <span>
        <strong>{point?.link ?? "Ready"}</strong>
        <small>{point?.point ?? "Awaiting launch"}</small>
      </span>
    </div>
  );
}
...
function StaticRoutePreview({ fallback = false }: { fallback?: boolean }) {
  return (
    <div
      className="route-map"
      role="img"
...
function DeliveryCompletionButton({
  ...
  if (!state.order || state.sortie.status !== "en_route") return null;
  return (
    <div className="delivery-complete-bar">
      <span>
        <PackageCheck size={18} aria-hidden="true" />
        <strong>Telemetry reached the fictional micro-hub.</strong>
      </span>
      <button
        type="button"
        ...
        onClick={() =>
          mutate(
            ...
            "Drop confirmed. Order delivered in the simulated journey.",
          )
        }
      >
        Confirm drop <Check size={15} aria-hidden="true" />
      </button>
    </div>
  );
}
...
function MarketsSurface() {
  return (
    <>
      <SectionTitle
        eyebrow="Market horizon"
        title="Start narrow. Build the corridor."
        detail="Active and planned market configuration"
      />
      <div className="panel markets-panel">
        <div className="market-horizon-intro">
          <div className="horizon-number">01</div>
          <div>
            <h3>Two active markets, one operating model.</h3>
            <p>
              Every new market needs its own currency, tax, duty, localization,
              port, and launch review. Francophone entry explicitly requires
              French localization.
            </p>
          </div>
        </div>
        <div className="horizon-table">
          {marketRows.map((row) => (
            ...
            >
              <span className="horizon-code">{row.code}</span>
              <span>
                <strong>{row.name}</strong>
                <small>{row.detail}</small>
              </span>
              <span>{row.phase}</span>
              <Badge tone={row.status === "Transacting" ? "green" : "neutral"}>
                {row.status}
              </Badge>
            </div>
          ))}
