...
function OriginBadge({ origin }: { origin: Product["origin"] }) { const labels = { direct_import: "Direct import", ghana_origin_export: "Ghana-origin export", marketplace_future: "Marketplace · future" }; return <Badge tone={origin === "ghana_origin_export" ? "green" : origin === "direct_import" ? "blue" : "amber"}>{labels[origin]}</Badge>; }
...
function LoadingBlock() { return <div className="loading-block" aria-label="Loading"><span /><span /><span /></div>; }
...
export default function PrototypeWorkspace({ initialRole = "consumer" }: { initialRole?: UserRole }) {
  ...
  async function mutate(url: string, options?: RequestInit, success?: string) {
    ...
    try { const response = await fetch(url, options); const body = await response.json() as { error?: string }; if (!response.ok) throw new Error(body.error ?? "The demo action failed"); await refresh(); if (success) setNotice(success); } catch (caught) { setError(caught instanceof Error ? caught.message : "The demo action failed"); } finally { setBusy(false); }
  }
  async function pay() {
    setBusy(true); setError(""); setNotice("");
    try { const init = await fetch("/api/payments/paystack/initialize", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ productId: selectedProductId, quantity }) }); const initBody = await init.json() as { error?: string; reference?: string }; if (!init.ok) throw new Error(initBody.error ?? "Payment could not initialize"); const verify = await fetch(`/api/payments/paystack/verify?reference=${encodeURIComponent(initBody.reference ?? "")}`); const verifyBody = await verify.json() as { error?: string }; if (!verify.ok) throw new Error(verifyBody.error ?? "Payment could not be verified"); await refresh(); setNotice("Payment verified server-side. The order is ready for fulfilment."); } catch (caught) { setError(caught instanceof Error ? caught.message : "Payment failed"); } finally { setBusy(false); }
  }
  ...
  async function switchRole(nextRole: UserRole) { setError(""); const response = await fetch("/api/demo/identity", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ role: nextRole }) }); if (!response.ok) { const body = await response.json() as { error?: string }; setError(body.error ?? "Identity switch failed"); return; } setRole(nextRole); setRoleOpen(false); }
  ...
  return <main className="workspace-frame">
    ...
    <div className="workspace-layout">
      <aside className="workspace-sidebar" aria-label="Prototype surfaces">
        ...
        {(Object.keys(surfaceLabels) as Surface[]).map((key) => <button key={key} type="button" className={surface === key ? "surface-nav active" : "surface-nav"} onClick={() => setSurface(key)}><span className="surface-icon">{key === "shop" ? <ShoppingBag size={16} aria-hidden="true" /> : key === "operations" ? <Warehouse size={16} aria-hidden="true" /> : key === "compliance" ? <FileCheck2 size={16} aria-hidden="true" /> : key === "delivery" ? <Plane size={16} aria-hidden="true" /> : <Map size={16} aria-hidden="true" />}</span><span>{surfaceLabels[key]}</span>{key === "shop" && <span className="nav-count">{state.cart.length || ""}</span>}</button>)}
      ...
      <section className="workspace-main" aria-live="polite">
        {notice && <div className="notice success"><Check size={16} aria-hidden="true" />{notice}<button type="button" onClick={() => setNotice("")} aria-label="Dismiss notice">×</button></div>}
        {error && <div className="notice error"><CircleAlert size={16} aria-hidden="true" /><span>{error}</span><button type="button" onClick={() => setError("")} aria-label="Dismiss error">×</button></div>}
        {surface === "shop" && <ShopSurface state={state} market={market} setMarket={changeMarket} products={filteredProducts} categories={categories} category={category} setCategory={setCategory} search={search} setSearch={setSearch} selectedProduct={selectedProduct} selectProduct={setSelectedProductId} quantity={quantity} setQuantity={setQuantity} pay={pay} busy={busy} isDeepOrder={isDeepOrder} />}
...
function ShopSurface({ state, market, setMarket, products, categories, category, setCategory, search, setSearch, selectedProduct, selectProduct, quantity, setQuantity, pay, busy, isDeepOrder }: { state: DemoState; market: "NG" | "GH"; setMarket: (value: "NG" | "GH") => void; products: Product[]; categories: string[]; category: string; setCategory: (value: string) => void; search: string; setSearch: (value: string) => void; selectedProduct: Product; selectProduct: (value: string) => void; quantity: number; setQuantity: (value: number) => void; pay: () => Promise<void>; busy: boolean; isDeepOrder: boolean }) {
  const quote = state.order ? { subtotalMinor: state.order.subtotalMinor, taxMinor: state.order.taxMinor, deliveryMinor: state.order.deliveryMinor, totalMinor: state.order.totalMinor } : { subtotalMinor: selectedProduct.priceMinor * quantity, taxMinor: Math.round(selectedProduct.priceMinor * quantity * .075), deliveryMinor: selectedProduct.origin === "ghana_origin_export" ? 450000 : 550000, totalMinor: Math.round(selectedProduct.priceMinor * quantity * 1.075) + (selectedProduct.origin === "ghana_origin_export" ? 450000 : 550000) };
  return <><div className="market-picker standalone"><label htmlFor="market-picker">Market</label><select id="market-picker" value={market} onChange={(event) => setMarket(event.target.value as "NG" | "GH")}><option value="NG">Nigeria · NGN</option><option value="GH">Ghana · GHS</option></select></div><SectionTitle eyebrow={`Commerce · ${market === "NG" ? "Nigeria" : "Ghana"}`} title="A storefront with a point of view." detail={`Rule-based demo curation · Local ${market === "NG" ? "NGN" : "GHS"} pricing · Provenance visible`} />
    <div className="commerce-grid">
      <div className="catalogue-column"><div className="shop-toolbar"><label className="search-box"><Search size={16} aria-hidden="true" /><span className="sr-only">Search products</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by product, maker, or provenance" /></label><div className="filter-row" role="group" aria-label="Product categories">{categories.map((item) => <button key={item} type="button" className={category === item ? "filter-chip active" : "filter-chip"} onClick={() => setCategory(item)}>{item}</button>)}</div></div><div className="product-grid">{products.map((product) => <button type="button" key={product.id} className={selectedProduct.id === product.id ? "product-card selected" : "product-card"} onClick={() => selectProduct(product.id)}><div className={`product-art art-${product.category.toLowerCase().replace(/[^a-z]/g, "-")}`}><span>{product.category === "Beauty" ? "01" : product.category === "Fashion" ? "02" : product.category === "Pantry" ? "03" : "04"}</span></div><div className="product-copy"><OriginBadge origin={product.origin} /><h3>{product.name}</h3><p>{product.producer}</p><strong>{product.priceMinor ? formatMoney(product.priceMinor, product.currency) : "Roadmap only"}</strong></div></button>)}</div>{products.length === 0 && <div className="panel-empty"><Search size={28} aria-hidden="true" /><strong>No products match this view</strong><p>Try another search or clear the category filter.</p></div>}</div>
      <div className="detail-column"><div className="detail-panel"><div className="detail-art"><span>{selectedProduct.category}</span><strong>{selectedProduct.name.split(" ").slice(0, 2).join(" ")}</strong></div><div className="detail-content"><OriginBadge origin={selectedProduct.origin} /><h3>{selectedProduct.name}</h3><p>{selectedProduct.description}</p><div className="detail-facts"><div><small>Producer</small><strong>{selectedProduct.producer}</strong></div><div><small>Fulfilment</small><strong>{selectedProduct.stockLabel}</strong></div><div><small>Batch / expiry</small><strong>{selectedProduct.batch} · {selectedProduct.expiry}</strong></div></div>{selectedProduct.transformation && <div className="evidence-strip"><ShieldCheck size={15} aria-hidden="true" /><span><strong>Transformation recorded</strong><small>{selectedProduct.transformation}</small></span></div>}{selectedProduct.ingredients && <p className="ingredients"><strong>Ingredients</strong> {selectedProduct.ingredients}</p>}<div className="purchase-row"><div className="quantity-control"><button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity">−</button><span>{quantity}</span><button type="button" onClick={() => setQuantity(Math.min(10, quantity + 1))} aria-label="Increase quantity">+</button></div><strong className="detail-price">{selectedProduct.priceMinor ? formatMoney(selectedProduct.priceMinor * quantity, selectedProduct.currency) : "Not available"}</strong></div>{selectedProduct.purchasable && <button className="primary-button wide" type="button" onClick={pay} disabled={busy || Boolean(state.order)}>{busy ? "Verifying payment…" : state.order ? "Order created · see tracking" : "Buy in test mode"}<CreditCard size={16} aria-hidden="true" /></button>}{selectedProduct.origin === "marketplace_future" && <div className="disabled-note">Marketplace settlement is future-only in this prototype.</div>}</div></div></div>
      {isDeepOrder && state.order && <OrderCard state={state} quote={quote} />}
    </div>
  </>;
}

function OrderCard({ state, quote }: { state: DemoState; quote: { subtotalMinor: number; taxMinor: number; deliveryMinor: number; totalMinor: number } }) { return <div className="order-card"><div className="order-card-header"><div><p className="eyebrow">Deep order · {state.order?.reference}</p><h3>Ghana → Nigeria, in one view.</h3></div><Badge tone="green">{state.order?.status.replace("_", " ")}</Badge></div><div className="order-price-lines"><span>Product <strong>{formatMoney(quote.subtotalMinor, "NGN")}</strong></span><span>Illustrative tax/duty treatment <strong>{formatMoney(quote.taxMinor, "NGN")}</strong></span><span>Delivery <strong>{formatMoney(quote.deliveryMinor, "NGN")}</strong></span><span className="total-line">Total <strong>{formatMoney(quote.totalMinor, "NGN")}</strong></span></div><div className="timeline">{state.orderEvents.map((event) => <div className={event.complete ? "timeline-item complete" : "timeline-item"} key={event.status}><span className="timeline-dot">{event.complete && <Check size={11} aria-hidden="true" />}</span><span><strong>{event.label}</strong><small>{event.detail}</small></span></div>)}</div></div>; }
